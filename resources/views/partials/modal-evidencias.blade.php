<div class="modal fade" id="modalEvidencias-{{ $solucion->idlista_soluciones_clientes }}" tabindex="-1" aria-hidden="true">
        ...
        <div class="modal-content shadow-lg border-0">
            ...
            <div class="modal-body">
                    ...
                    @forelse($solucion->evidencias as $evidencia)
                        <div class="col-md-4 mb-4">
                            <div class="card h-100 shadow-sm border-0">
                                <img src="{{ asset('storage/' . $evidencia->archivo) }}" class="card-img-top rounded"
                                    style="max-height: 150px; object-fit: cover;">

                                @can('solucionescliente.editar')
                                    <div class="card-body text-center">
                                        <form method="POST" action="{{ route('evidencias.destroy', [$solucion->idlista_soluciones_clientes, $evidencia->id]) }}">
                                            @csrf
                                            @method('DELETE')
                                            <button class="btn btn-outline-danger btn-sm w-100">
                                                <i class="bi bi-trash"></i> Eliminar  {{ $evidencia->id }}
                                            </button>
                                        </form>
                                    </div>
                                @endcan
                ...
                @if($solucion->evidencias->isEmpty())
                    ...
                    <form action="{{ route('evidencias.store', $solucion->idlista_soluciones_clientes) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                ...
                @if($solucion->evidencias->isNotEmpty())
                    @cannot('solucionescliente.editar')
                        <hr class="my-4">
                        <p class="text-muted fst-italic">⚠️ Ya se han registrado evidencias para esta solución. Solo puedes visualizarlas.</p>
                    @endcannot
                @endif
                ...
                @can('solucionescliente.editar')
                    @if($solucion->evidencias->isNotEmpty())
                        ...
                        <form action="{{ route('evidencias.update', $solucion->idlista_soluciones_clientes) }}" method="POST" enctype="multipart/form-data">
                            @csrf
                            @method('PUT')
