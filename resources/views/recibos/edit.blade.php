...
@section('content')
@include('layouts.navbars.auth.topnav', ['title' => 'Editar Recibo de Devolución'])

<div class="container-fluid py-4">
  <div class="row">
    <div class="col-12">
      <div class="card mb-4 shadow">
        <h1 class="text-center">✏️ Editar Recibo de Devolución</h1>
        @if(session('success'))
        ...
        <div class="card">
          <div class="card-header"><h5>📋 Detalles del Recibo</h5></div>
          <div class="card-body">
            <form action="{{ route('recibos.update', $recibo->idrecibos) }}" method="POST" enctype="multipart/form-data">
              @csrf
              @method('PUT')
              ...
              <div class="mb-3">
                ...
                <select name="provedores_idprovedores" class="form-select" required>
                  ...
                  @foreach($proveedores as $prov)
                    <option value="{{ $prov->idcatalogo_provedores }}"
                      {{ $prov->idcatalogo_provedores == $recibo->provedores_idprovedores ? 'selected' : '' }}>
              ...
              <div class="form-check form-switch">
                <input type="checkbox" name="devolucion" class="form-check-input" id="devolucion" value="1"
                  {{ optional($recibo->conceptosEstado->first())->devolucion == 1 ? 'checked' : '' }}>
              ...
              <div id="factura-wrapper">
                @foreach($recibo->concepto_recibos as $i => $concepto)
                  <div class="factura-item card mb-3 border-start border-3 border-secondary p-3 position-relative {{ $i %2 ==0 ? 'bg-light' : 'bg-white' }}">
                    <div class="row gx-2 align-items-end">
                      <div class="col-md-2">
                        <label>Factura</label>
                        <input name="facturas[{{ $i }}][factura]" value="{{ $concepto->numero_factura }}" required class="form-control">
                      </div>
                      <div class="col-md-1">
                        <label>Cantidad</label>
                        <input name="facturas[{{ $i }}][cantidad]" type="number" value="{{ $concepto->cantidad }}" required class="form-control">
                      </div>
                      ...
                      <div class="col-md-6">
                        ...
                        <input name="facturas[{{ $i }}][descripcion]"
                          value="{{ $concepto->producto->idescrip ?? '' }}" class="form-control" disabled>
                      </div>
                      <div class="col-md-2">
                        <label>Observaciones</label>
                        <select name="facturas[{{ $i }}][observaciones]" class="form-select">
                            <option value="No Solicitado" {{ $concepto->observaciones=='No Solicitado'?'selected':'' }}>No Solicitado</option>
                            <option value="Multiplo Incompleto" {{ $concepto->observaciones=='Multiplo Incompleto'?'selected':'' }}>Multiplo Incompleto</option>
                            <option value="Mal Empacado" {{ $concepto->observaciones=='Mal Empacado'?'selected':'' }}>Mal Empacado</option>
                            <option value="Error de compras" {{ $concepto->observaciones=='Error de compras'?'selected':'' }}>Error de compras</option>
                            <option value="Empaque dañado" {{ $concepto->observaciones=='Empaque dañado'?'selected':'' }}>Empaque dañado</option>
                            <option value="Empaque roto" {{ $concepto->observaciones=='Empaque roto'?'selected':'' }}>Empaque roto</option>
                            <option value="Etiqueta No Visible" {{ $concepto->observaciones=='Etiqueta No Visible'?'selected':'' }}>Etiqueta No Visible</option>
                        </select>
                      </div>
                    </div>
    ...
    const row = `
      <div class="factura-item card mb-3 border-start border-3 border-secondary p-3 position-relative ${facturaIndex%2==0?'bg-light':'bg-white'}">
        <div class="row gx-2 align-items-end">
          ...
          <div class="col-md-2"><label>Observaciones</label><select name="facturas[${facturaIndex}][observaciones]" class="form-select"><option value="">Observación</option><option value="Empaque dañado o roto">Empaque dañado o roto</option><option value="No Solicitado">No Solicitado</option><option value="Multiplo Incompleto">Multiplo Incompleto</option><option value="Mal Empacado">Mal Empacado</option></select></div>
        </div>
        <span class="remove-factura text-danger position-absolute top-0 end-0 p-2" style="cursor:pointer;">🗑️</span>
      </div>`;
